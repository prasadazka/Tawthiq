...
async def validate_document(
    ...
    """Run rules-only validation. Table extraction is now a separate, lazy
    endpoint (/api/pdf-tables/extract) triggered when the user opens the
    PDF Tables sub-tab in the results view."""
    t_total = time.time()
    ...
    # Step 3: Run rules — batched Gemini calls
    t0 = time.time()
